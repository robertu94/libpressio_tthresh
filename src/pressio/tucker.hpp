        ...
        for (uint32_t i = 0; i < slice.get_size(); ++i) {
            switch (slice.reduction) {
                ...
                case Box: {
                    ...
                    int32_t end = max<int32_t>(min<int32_t>(slice.points[0] + i*slice.points[2] + (slice.points[2] - slice.points[2]/2), U.rows()), 0);
                ...
                case Lanczos: {
                    ...
                    int32_t start = max<int32_t>(min<int32_t>(slice.points[0] + i*slice.points[2] - a, U.rows()-1), 0); // Kernel support: [-a, a], clamped
                    int32_t end = max<int32_t>(min<int32_t>(slice.points[0] + i*slice.points[2] + a + 1, U.rows()), -1);
